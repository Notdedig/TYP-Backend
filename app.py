...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  .db{background:#2e294e;padding:14px;font-family:var(--font-sans,sans-serif);color:#efbcd5}
  ...
  .cc{background:#3a3560;border:0.5px solid #8661c1;border-radius:10px;padding:11px 13px}
  .cc .lbl{font-size:10px;color:#be97c6;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:7px}
  ...
  .btn{background:#8661c1;color:#fff;border:none;border-radius:7px;padding:5px 12px;font-size:11px;cursor:pointer;transition:background 0.15s}
  .btn:hover{background:#9e7ad4}
  .btn:disabled{opacity:0.5;cursor:not-allowed}
  .btn.ok{background:#3a8c6e}.btn.ok:hover{background:#4aad89}
  .btn.danger{background:#c0446a}.btn.danger:hover{background:#d4557c}
  ...
  .log{background:#1a1735;border-radius:7px;padding:8px 10px;font-size:10px;color:#be97c6;margin-top:8px;min-height:28px;line-height:1.6}
</style>

<div class="db" role="main">
  <h2 style="position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0)">Cognitive Load Dashboard — posts simulated vitals to real API, reads back cognitive load in real time.</h2>
  ...
  <div class="crow">
    <div class="cc">
      ...
      <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:#be97c6">
        ...
        <span class="badge b-sim" id="modeBadge" style="margin-left:auto">Sim only</span>
      </div>
      <div class="log" id="apiLog">Waiting…</div>
    </div>

    <div class="cc">
      <div class="lbl">Vitals simulator</div>
      <div class="mode-row">
        <span class="mode-lbl">Post to API</span>
        <label class="tog">
          <input type="checkbox" id="postToggle">
          <span class="tsl"></span>
        </label>
        <span class="mode-lbl" id="postLbl" style="color:#5a5280">OFF</span>
      </div>
      <div class="srow">
  ...
  <div class="chart-card">
    <div class="ct">
      ...
      <span class="badge b-sim" id="clBadge" style="margin-left:auto">Local calc</span>
    </div>
    <div style="position:relative;width:100%;height:220px">
      <canvas id="loadChart" role="img" aria-label="Real-time cognitive load Paas 1-9">Real-time cognitive load.</canvas>
    </div>
  </div>
  ...
  <div class="chart-card">
    ...
    <div style="position:relative;width:100%;height:170px">
      <canvas id="vitalsChart" role="img" aria-label="Real-time heart rate and breath rate">Real-time vitals.</canvas>
    </div>
  </div>
</div>
...
<script>
const MAX_PTS = 60;
const WH = 0.6, WB = 0.4, MAX_DELTA = 0.15;
...
const loadChart = new Chart(document.getElementById('loadChart').getContext('2d'),{
  ...
  options:{responsive:true,maintainAspectRatio:false,animation:false,
    plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>` ${c.dataset.label}: ${c.parsed.y!=null?c.parsed.y.toFixed(1):'—'}`}}},
...
document.getElementById('postToggle').addEventListener('change',function(){
  postToApi=this.checked;
  document.getElementById('postLbl').textContent=postToApi?'ON':'OFF';
  document.getElementById('postLbl').style.color=postToApi?'#efbcd5':'#5a5280';
  if(postToApi && !apiOk){ log('Test the API connection first'); this.checked=false; postToApi=false; document.getElementById('postLbl').textContent='OFF'; document.getElementById('postLbl').style.color='#5a5280'; }
});
...
async function testApi(){
  ...
  el.innerHTML='<span class="sdot am"></span>Testing…';
  log('Testing /health…');
  try{
    const r=await fetch(apiBase+'/health',{signal:AbortSignal.timeout(4000)});
    if(r.ok){
      el.innerHTML='<span class="sdot g"></span>Connected';
      apiOk=true;
      document.getElementById('modeBadge').className='badge b-live';
      document.getElementById('modeBadge').textContent='API ready';
      log('Connected — flip "Post to API" to start sending');
    } else { throw new Error('HTTP '+r.status); }
  } catch(e){
    el.innerHTML='<span class="sdot r"></span>Unreachable';
    apiOk=false;
    log('Error: '+e.message);
  }
}
...
async function startCal(){
  ...
  log('Posting to /api/calibrate/start…');
  try{
    const r=await fetch(apiBase+'/api/calibrate/start',{method:'POST',headers:{'Content-Type':'application/json'}});
    const d=await r.json();
    log('Calibration started on server — 90s');
...
async function postVitals(hr, br){
  await Promise.all([
...
async function fetchLoad(){
  ...
  return {hrR,brR,clR,predR};
}

function clamp(v,a,b){return Math.max(a,Math.min(b,v));}
function calcPredLocal(cur){
  recentLoads.push(cur); if(recentLoads.length>10)recentLoads.shift();
  if(recentLoads.length>=6){
    const ra=recentLoads.slice(-3).reduce((a,b)=>a+b,0)/3;
    const oa=recentLoads.slice(-6,-3).reduce((a,b)=>a+b,0)/3;
    return clamp(cur+(ra-oa)*0.5,1,9);
  }
  return cur;
}
...
async function tick(){
  const hr = curHR + (Math.random()*1.6-0.8);
  const br = curBR + (Math.random()*0.5-0.25);
  const hrR = Math.round(hr), brR = Math.round(br);

  document.getElementById('hrDisp').textContent=hrR;
  document.getElementById('brDisp').textContent=brR;
  push(dHR,hrR); push(dBR,brR);

  let cl=null, pl=null;

  if(postToApi && apiOk){
    try{
      await postVitals(hrR,brR);
      const res=await fetchLoad();
      cl=res.clR.currentCognitiveLoad;
      pl=res.predR.predictedCognitiveLoad;
      const isCal=res.clR.isCalibrated;
      const bhr=res.hrR.baselineHeartRate, bbr=res.brR.baselineBreathRate;
      document.getElementById('hrBase').textContent=bhr?bhr.toFixed(0):'—';
      document.getElementById('brBase').textContent=bbr?bbr.toFixed(0):'—';
      document.getElementById('hrDelta').textContent='Δhr '+res.clR.heartRateDelta.toFixed(2);
      ...
      document.getElementById('vBadge').textContent='Posted to API';
      log(`POST ok · HR ${hrR} BR ${brR} → Load ${cl!=null?cl.toFixed(1):'—'}`);
    } catch(e){
      log('Fetch error: '+e.message);
      document.getElementById('vBadge').className='badge b-err';
      document.getElementById('vBadge').textContent='API error';
    }
  } else {
    document.getElementById('clBadge').className='badge b-sim';
    document.getElementById('clBadge').textContent='Local calc';
    document.getElementById('vBadge').className='badge b-sim';
    document.getElementById('vBadge').textContent='Simulated';
  }

  push(dCur, cl!=null?cl:null);
  push(dPred, pl!=null?pl:calcPredLocal(cl??1));

  if(cl!=null){
    document.getElementById('curLoad').textContent=cl.toFixed(1);
    document.getElementById('predLoad').textContent=(pl??calcPredLocal(cl)).toFixed(1);
    document.getElementById('curBar').style.width=((cl-1)/8*100).toFixed(1)+'%';
    document.getElementById('predBar').style.width=(((pl??cl)-1)/8*100).toFixed(1)+'%';
  }
...
def home():
    ...
    return jsonify({
        ...
        "endpoints": {
            ...
            "GET /api/dashboard": "Serve the cognitive load dashboard"
        }
    })
...
def dashboard():
    """Serve the cognitive load dashboard"""
